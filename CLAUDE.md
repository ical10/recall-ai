...
- one repo, two services. each service has its own build config (skips the Node + Tailwind build for the cron service — faster deploys, smaller images) and deploy config (so only web runs `alembic upgrade head` pre-deploy).
  - **web**: installs python + uv + nodejs + pnpm, runs `pnpm install --frozen-lockfile`, `uv sync`, then `pnpm run build` to build the React SPA into `apps/web/dist`. preDeployCommand runs `alembic upgrade head`. Start: uvicorn, which serves the api under `/api` + `/auth` and the built SPA (via `apps/web/dist`) for every other path. `/healthz` healthcheck.
