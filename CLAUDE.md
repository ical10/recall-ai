...
- required env vars: `DATABASE_URL`, `REDIS_URL`, `LLM_API_KEY`, `SECRET_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. optional: `LLM_MODEL` (defaults to `z-ai/glm-4.5-air:free` — override per environment, e.g. prod = `deepseek/deepseek-v4-flash` via OpenRouter, or `deepseek-v4-flash` via OpenCode Go), `LLM_BASE_URL` (defaults to `https://openrouter.ai/api/v1`; swap for OpenCode Go's `https://opencode.ai/zen/go/v1` or DeepSeek-direct `https://api.deepseek.com/v1` with no code change). all come from railway env / shared variables. railway's postgres + redis addons inject `DATABASE_URL` and `REDIS_URL` automatically when attached.
...
- chose openai sdk + provider-neutral settings (2026-05-04, refined 2026-05-07; supersedes earlier may-2026 anthropic call): openrouter, opencode go, deepseek-direct, and openai are all OpenAI-API-compatible, so the openai python sdk works against any of them by changing only `LLM_BASE_URL` + `LLM_API_KEY` + `LLM_MODEL` env vars (no code change). dev defaults to OpenRouter + `z-ai/glm-4.5-air:free` (smoke-tested 2026-05-07: produced valid structured output first-try; the earlier `meta-llama/llama-3.3-70b-instruct:free` default returns 402 from upstream). prod options: OpenRouter @ `deepseek/deepseek-v4-flash` (~$0.14 in / $0.28 out per Mtok, pay-per-token, smoke-tested 2026-05-07 first-try); or OpenCode Go @ `deepseek-v4-flash` if user already has the subscription and call volume fits its quota. retry-with-prompt-refinement loop absorbs the lower instruction-following quality of free models.
