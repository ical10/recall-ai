...
- required env vars: `DATABASE_URL`, `REDIS_URL`, `OPENROUTER_API_KEY`, `SECRET_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. optional: `LLM_MODEL` (defaults to a free OpenRouter model — override per environment, e.g. prod = `deepseek/deepseek-v4-flash`), `OPENROUTER_BASE_URL` (defaults to `https://openrouter.ai/api/v1`). all come from railway env / shared variables. railway's postgres + redis addons inject `DATABASE_URL` and `REDIS_URL` automatically when attached.
...
- chose openai sdk targeting openrouter over anthropic sdk (2026-05-04, supersedes earlier may-2026 anthropic call): openrouter is openai-api-compatible so the openai python sdk works as the client; gives access to `:free` models for dev/staging while letting prod swap to any paid model (openai, anthropic, google, deepseek) by changing the `LLM_MODEL` env var with no code change. dev defaults to `z-ai/glm-4.5-air:free` (smoke-tested 2026-05-07: produced valid structured output first-try; the earlier `meta-llama/llama-3.3-70b-instruct:free` default returns 402 from upstream). prod target is `deepseek/deepseek-v4-flash` (~$0.14 in / $0.28 out per Mtok, smoke-tested 2026-05-07: valid first-try). retry-with-prompt-refinement loop absorbs the lower instruction-following quality of free models.
